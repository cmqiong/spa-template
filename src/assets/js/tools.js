...
if (process.env === 'development') {
  ...
  host = 'http://wx.dev.sunbar.cn:8081';
...
export function ajax(config) {
  ...
  const httpUrl = (options.url.indexOf('http://') > -1 || options.url.indexOf('https://') > -1) ? options.url : `${host}${options.url}`;
  $.ajax({
    url: httpUrl,
    type: options.type,
    data: options.data,
...
// 有不同的 url 前缀时调用此方法
export function ajaxPrefixGet(prefix, getUrl, getData = {}) {
  // ajaxGet(getUrl, getData = {});
}
